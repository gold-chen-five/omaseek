// The key lookup (ctrl+k): every key in the panel as `key — what it does`,
// grouped by where it works, and the words typed to find one. The rebindable
// keys come from ACTIONS with whatever they are bound to now; the rest are the
// fixed keys the settings page lists. Indexed loops and indexOf only: this runs
// in QML's JS engine too.
...
/** [{ group, keys, text }]: every key, as bound under `settings`, in GROUP_ORDER. */
export function keyEntries (settings) {
  ...
  if (escape) entries.push({ group: 'Field', keys: escape, text: 'Leave insert with — typed quickly, back to normal mode' })
  ...
  for (let i = 0; i < FIXED_KEYS.length; i++) {
    ...
    for (let r = 0; r < runs.length; r++) {
      ...
      for (let p = 0; p < parts.length; p++) entries.push({ group: FIXED_KEYS[i].label, keys: '', text: parts[p] })
    }
  }
...
/**
 * The entries every typed word appears in — in the key, what it does, or its
 * group — whatever the case. An empty query keeps them all.
 */
export function filterEntries (entries, query) {
  ...
  for (let i = 0; i < entries.length; i++) {
    const hay = (entries[i].keys + ' ' + entries[i].text + ' ' + entries[i].group).toLowerCase()
    let all = true
