...
export const FIXED_KEYS = [
  ...
  { label: 'Field', keys: 'insert: ctrl+w ctrl+u delete back · ctrl+j new line (ask) · ↑ ↓ lines (ask), past queries (search) · normal: vim motions, L H the next saved conversation and the one before (ask), gx opens the URL under the cursor, q or esc stops a reply being written (ask), o O open line (ask), f{char} then f/F repeats, r{char}, d c y, text objects, v V, p P, u ctrl+r, counts' },
  ...
  { label: 'Answer', keys: 'q stops a reply being written · L H the next saved conversation and the one before (3L walks three) · h j k l w b e 0 ^ _ $ move · f t ; , find · v V select · gv reselect · y{motion} yy yank · p P put in the ask bar · / ? n N search · * # the word under the cursor · gn field normal · gi i a field insert' },
