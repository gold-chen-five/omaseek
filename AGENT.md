...
| `docs/` | the README's screenshot, `demo.png`. The demo video is not here: GitHub plays a video only when it is uploaded through the website (a `user-attachments` link in the README), and a committed one would be cloned by every install |
