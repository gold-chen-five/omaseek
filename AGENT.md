...
| `preview.png` | the screenshot heading the README, and the plugin marketplace's card and detail image — it reads a root `preview.png`. The demo video is not in the repository: GitHub plays one only when it is uploaded through the website (the `user-attachments` link in the README), and a committed one would be cloned by every install |
...
- `settings/keylist.mjs` — the ctrl+k lookup's entries, each shaped like a settings row (`label`, `keys`, `hint`, `fixed`): the Keys section as the page lists it, then `FIXED_KEYS` one row per group, filtered by every typed word
