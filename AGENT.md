...
| `src/settings/` | `SettingsPage`, `SettingRow`, `SettingsDropdown`, `SettingsRows`, `ConfigStore`, `SettingsActions`, `Shortcut`, `KeysLookup` (ctrl+k: the page's own key rows, read-only, over a `VimTextField` filter), and the settings modules |
...
- `settings/keylist.mjs` — the ctrl+k lookup's entries, each shaped like a settings row (`label`, `keys`, `hint`, and `fixed` for a line with no binding): `ACTIONS` as bound now plus `FIXED_KEYS` split into lines, grouped, and filtered by every typed word
