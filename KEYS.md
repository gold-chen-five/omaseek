...
`ctrl+v` as any text box does. `ctrl+c` is New session, the panel's own
interrupt, never copy. Omarchy's `super+v` sends `ctrl+v` to a panel, so it
pastes here; its `super+c` sends `ctrl+c`, so here it starts a new
conversation — copy with `ctrl+shift+c`. None of the three can be rebound, and
settings refuses them for another key.
