...
| `ctrl+k` (Look up keys) | every key in the panel, row for row as Settings → Keys lists them — what it does, the key beside it, the hint under it — grouped by where it works and showing your current bindings, with each group's fixed vim keys under it. The filter is the panel's own field, so every Vim key edits what you type (`jk`, `db`, `ciw`, `u`); any words, in any case, filter (`trans`, `ctrl+x`, `undo`). `j` `k` and `↓` `↑` walk the list; `esc` leaves insert, `esc` again or `ctrl+k` closes it and the keyboard goes back where it was, in the mode it was in |
